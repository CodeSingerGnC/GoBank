...
func createRandomAccount(t *testing.T) Account {
	...
	var account Account
	query := `SELECT id, owner, balance, currency, created_at FROM accounts WHERE id = ? LIMIT 1`
	err = testdb.QueryRow(query, lastInsertId).Scan(&account.ID, &account.Owner, &account.Balance, &account.Currency, &account.CreatedAt)
	require.NoError(t, err)
	require.Equal(t, lastInsertId, account.ID)
