...
// -TODO: 需要深入理解 mock 代码和表驱动测试
func TestGetAccountAPI(t *testing.T) {
	user, _ := randomUser(t)
	account := randomAccount(user.UserAccount)
	...
	testCase := []struct {
	...
	}{
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user.UserAccount, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
